...
Ordering (#63) is five tiers, then alphabetical: **match band** (exact symbol > symbol prefix > name-only) → **tradeable** (USD on a US exchange, i.e. rows `DirectoryResolver` would accept, before ones it would 422) → **live** (`end_date` within 180 days or NULL, before delisted) → **asset class** (equities/ETFs before mutual funds) → **symbol length**. The cap makes ordering a correctness concern, not a nicety: before this, `MSF` never returned MSFT and `AAP` never returned AAPL. **Two-character queries remain unable to surface some mega-caps** (`AA` still misses AAPL) — more than 20 live tradeable non-fund symbols share those prefixes and the directory carries no popularity signal; at three characters they are all reachable.
