 ...
 * No `id` is returned (transactions POST by symbol); max 20. Ordering is six
 * tiers then alphabetical — match band, tradeable, live, asset class, symbol
 * length, listing age (issues #63/#71; see docs/API_SHAPES.md). Ordering
 * matters because the result set is CAPPED: a weak sort silently truncates the
 * row the user meant rather than returning a wrong one. The server owns this
 * entirely — do not re-sort results client-side, or the cap's contents and the
 * displayed order will disagree.
 *
